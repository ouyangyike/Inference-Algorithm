...
def layerBuild(X,hidden_W,hidden_b):
    ...
    Z = tf.matmul(X,tf.nn.dropout(hidden_W,0.135554221007)) + hidden_b
...
def runNN(learningRate, batch_size, epoch, lamda, hidden_number):
    ...
    for step in range(0, epoch):
        ...
        y_valid_predicted, valid_loss = sess.run(forwardNN(validData, currentW, currentb, 0.135554221007*current_hidden_W, current_hidden_b, validTarget, lamda))
        valid_error = sess.run(evaluate(validTarget, y_valid_predicted))

        # test set prediction
        y_test_predicted, test_loss = sess.run(forwardNN(testData, currentW, currentb, 0.135554221007*current_hidden_W, current_hidden_b, testTarget, lamda))
