...
def forwardNN(X, W, b, hidden_W1, hidden_b1, hidden_W2, hidden_b2, hidden_W3, hidden_b3, hidden_W4, hidden_b4, hidden_W5, hidden_b5, y_target, lamda):
    ...
    #fifth hidden layer
    Z5 = layerBuild(activation4, hidden_W5, hidden_b5)

    #fifth hidden layer activation
    activation5 = tf.nn.relu(Z5)

    # Output layer: Graph and Error definition
    y_predicted, crossEntropy = softmax(activation5, W, b, y_target, lamda)

    return y_predicted, crossEntropy

def buildGraph(learingRate, lamda, hidden_number1, hidden_number2, hidden_number3, hidden_number4, hidden_number5):
    # Variable creation

    W = tf.Variable(tf.truncated_normal(shape=[hidden_number5, 10], stddev=3.0/(1 + hidden_number4),dtype=tf.float64,seed=19930107), name='weights')
    ...
    hidden_W5 = tf.Variable(tf.truncated_normal(shape=[hidden_number4, hidden_number5], stddev=3.0 / (hidden_number4 + hidden_number5),dtype=tf.float64, seed=19930107), name='hidden_weights5')
    hidden_b5 = tf.Variable(tf.zeros_initializer(shape=[1, hidden_number5], dtype=tf.float64), name='hidden_biases5')

    # Graph and Error definition
    y_predicted, crossEntropy = forwardNN(X, W, b, hidden_W1, hidden_b1, hidden_W2, hidden_b2, hidden_W3, hidden_b3, hidden_W4, hidden_b4, hidden_W5, hidden_b5, y_target, lamda)

    # Training mechanism
    optimizer = tf.train.AdamOptimizer(learning_rate=learingRate)
    train = optimizer.minimize(loss=crossEntropy)
    return W, b, hidden_W1, hidden_b1, hidden_W2, hidden_b2, hidden_W3, hidden_b3, hidden_W4, hidden_b4, hidden_W5, hidden_b5, X, y_target, y_predicted, crossEntropy, train
...
def runNN(learningRate, batch_size, epoch, lamda, hidden_number1, hidden_number2, hidden_number3, hidden_number4, hidden_number5):
    # Build computation graph
    W, b, hidden_W1, hidden_b1, hidden_W2, hidden_b2, hidden_W3, hidden_b3, hidden_W4, hidden_b4, hidden_W5, hidden_b5, X, y_target, y_predicted, crossEntropy, train = buildGraph(learningRate, lamda, hidden_number1, hidden_number2, hidden_number3, hidden_number4, hidden_number5)
    ...
    initial_hidden_W5 = sess.run(hidden_W5)
    initial_hidden_b5 = sess.run(hidden_b5)
    ...
    for step in range(0, epoch):
        ...
        for i in range(0, len(trainTarget) // batch_size):
            _, train_loss, currentW, currentb, current_hidden_W1, current_hidden_b1, current_hidden_W2, current_hidden_b2, current_hidden_W3, current_hidden_b3, current_hidden_W4, current_hidden_b4, current_hidden_W5, current_hidden_b5, y_train_predicted = sess.run([train, crossEntropy, W, b, hidden_W1, hidden_b1, hidden_W2, hidden_b2, hidden_W3, hidden_b3, hidden_W4, hidden_b4, hidden_W5, hidden_b5, y_predicted],feed_dict={X: trainData_random[(i * batch_size):((i + 1) * batch_size), :],y_target: trainTarget_random[(i * batch_size):((i + 1) * batch_size),:]})
            #train_error = sess.run(evaluate(trainTarget_random[(i * batch_size):((i + 1) * batch_size), :], y_train_predicted))

        # valid set prediction
        y_valid_predicted, valid_loss = sess.run(forwardNN(validData, currentW, currentb, current_hidden_W1, current_hidden_b1, current_hidden_W2, current_hidden_b2, current_hidden_W3, current_hidden_b3, current_hidden_W4, current_hidden_b4, current_hidden_W5, current_hidden_b5, validTarget, lamda))
        valid_error = sess.run(evaluate(validTarget, y_valid_predicted))

        # test set prediction
        y_test_predicted, test_loss = sess.run(forwardNN(testData, currentW, currentb, current_hidden_W1, current_hidden_b1, current_hidden_W2, current_hidden_b2, current_hidden_W3, current_hidden_b3, current_hidden_W4, current_hidden_b4, current_hidden_W5, current_hidden_b5, testTarget, lamda))
