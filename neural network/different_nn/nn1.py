...
def layerBuild(X,hidden_W,hidden_b):
    ...
    Z = tf.matmul(X,tf.nn.dropout(hidden_W,0.988626525163)) + hidden_b
...
def runNN(learningRate, batch_size, epoch, lamda, hidden_number1, hidden_number2, hidden_number3, hidden_number4):
    ...
    for step in range(0, epoch):
        ...
        y_valid_predicted, valid_loss = sess.run(forwardNN(validData, currentW, currentb, 0.988626525163*current_hidden_W1, current_hidden_b1, 0.988626525163*current_hidden_W2, current_hidden_b2, 0.988626525163*current_hidden_W3, current_hidden_b3, 0.988626525163*current_hidden_W4, current_hidden_b4, validTarget, lamda))
        valid_error = sess.run(evaluate(validTarget, y_valid_predicted))

        # test set prediction
        y_test_predicted, test_loss = sess.run(forwardNN(testData, currentW, currentb, 0.988626525163*current_hidden_W1, current_hidden_b1, 0.988626525163*current_hidden_W2, current_hidden_b2, 0.988626525163*current_hidden_W3, current_hidden_b3, 0.988626525163*current_hidden_W4, current_hidden_b4, testTarget, lamda))
