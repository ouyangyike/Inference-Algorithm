...
def forwardNN(X, W, b, hidden_W1, hidden_b1,  hidden_W2, hidden_b2, y_target, lamda):
    ...
    y_predicted, crossEntropy = softmax(activation2, W, b, y_target, lamda)

    return y_predicted, crossEntropy

def buildGraph(learingRate, lamda, hidden_number1, hidden_number2):
    # Variable creation

    W = tf.Variable(tf.truncated_normal(shape=[hidden_number2, 10], stddev=3.0/(1 + hidden_number2),dtype=tf.float64), name='weights')
    ...
    hidden_W1 = tf.Variable(tf.truncated_normal(shape=[784, hidden_number1],stddev=3.0/(784 + hidden_number1),dtype=tf.float64), name='hidden_weights1')
    hidden_b1 = tf.Variable(tf.zeros_initializer(shape=[1, hidden_number1], dtype=tf.float64),name='hidden_biases1')
    hidden_W2 = tf.Variable(tf.truncated_normal(shape=[hidden_number1, hidden_number2], stddev=3.0 / (hidden_number1+ hidden_number2), dtype=tf.float64),name='hidden_weights2')
    ...
    y_predicted, crossEntropy = forwardNN(X, W, b, hidden_W1, hidden_b1, hidden_W2, hidden_b2, y_target, lamda)

    # Training mechanism
    optimizer = tf.train.AdamOptimizer(learning_rate=learingRate)
    train = optimizer.minimize(loss=crossEntropy)
    return W, b, hidden_W1, hidden_b1, hidden_W2, hidden_b2, X, y_target, y_predicted, crossEntropy, train
...
def runNN(learningRate, batch_size, epoch, lamda, hidden_number1, hidden_number2):
    # Build computation graph
    W, b, hidden_W1, hidden_b1, hidden_W2, hidden_b2, X, y_target, y_predicted, crossEntropy, train = buildGraph(learningRate, lamda, hidden_number1, hidden_number2)
    ...
    for step in range(0, epoch):
        ...
        for i in range(0, len(trainTarget) // batch_size):
            _, train_loss, currentW, currentb, current_hidden_W1, current_hidden_b1, current_hidden_W2, current_hidden_b2, y_train_predicted = sess.run([train, crossEntropy, W, b, hidden_W1, hidden_b1, hidden_W2, hidden_b2, y_predicted],feed_dict={X: trainData_random[(i * batch_size):((i + 1) * batch_size), :],y_target: trainTarget_random[(i * batch_size):((i + 1) * batch_size),:]})
            train_error = sess.run(evaluate(trainTarget_random[(i * batch_size):((i + 1) * batch_size), :], y_train_predicted))

            # # valid set prediction
            y_valid_predicted, valid_loss = sess.run(forwardNN(validData, currentW, currentb, current_hidden_W1, current_hidden_b1, current_hidden_W2, current_hidden_b2, validTarget, lamda))
            valid_error = sess.run(evaluate(validTarget, y_valid_predicted))

            # test set prediction
            y_test_predicted, test_loss = sess.run(forwardNN(testData, currentW, currentb, current_hidden_W1, current_hidden_b1, current_hidden_W2, current_hidden_b2, testTarget, lamda))
