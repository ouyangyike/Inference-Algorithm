...
def layerBuild(X,hidden_W,hidden_b):
    ...
    Z = tf.matmul(X,tf.nn.dropout(hidden_W,0.5)) + hidden_b
...
def runNN(learningRate, batch_size, epoch, lamda, hidden_number):
    ...
    # Training model
    logging_hidden_w = np.zeros((epoch, 784, hidden_number))
    ...
    for step in range(0, epoch):
        ...
        # y_valid_predicted, valid_loss = sess.run(forwardNN(validData, currentW, currentb, 0.5*current_hidden_W, current_hidden_b, validTarget, lamda))
        # valid_error = sess.run(evaluate(validTarget, y_valid_predicted))
        #
        # # test set prediction
        # y_test_predicted, test_loss = sess.run(forwardNN(testData, currentW, currentb, 0.5*current_hidden_W, current_hidden_b, testTarget, lamda))
        # test_error = sess.run(evaluate(testTarget, y_test_predicted))


        logging_hidden_w[step] = current_hidden_W

    return logging_hidden_w

def plotNNFilter(units):
    units = np.reshape(units.T,(1000,28,28))
    n_columns = 10
    n_rows = 10
    for j in range(10):
        plt.figure(j+1)
        for i in range(100):
            ...
            plt.imshow(units[j*100+i,:,:], interpolation="nearest", cmap="gray")
        plt.show()