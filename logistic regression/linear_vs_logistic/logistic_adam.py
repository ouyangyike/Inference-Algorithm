...
def CE(X,W,b,y_target):
    # compute the linear regression function and mean squared error
    y_predicted = tf.matmul(X,W)+b
    crossEntropy = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=y_predicted, targets=y_target))
...
def buildGraph(learingRate):
    ...
    y_predicted, crossEntropy = CE(X,W,b,y_target)
...
def runLogistic(learningRate,batch_size, epoch):

    # Build computation graph
    W, b, X, y_target, y_predicted, crossEntropy, train = buildGraph(learningRate)
    ...
    for step in xrange(0,epoch):
        ...
        for i in xrange(0,len(trainTarget)/batch_size):
            ...
            y_valid_predicted, valid_err = sess.run(CE(validData,currentW,currentb,validTarget))
            valid_accuracy = evaluate(validTarget, y_valid_predicted)

            #test set prediction
            y_test_predicted, test_err = sess.run(CE(testData,currentW,currentb,testTarget))
